...
<template>
  <div class="home-view bg-background-light min-h-screen">
    <!-- Hero Section -->
    <section class="relative bg-gradient-to-r from-primary-red to-secondary-red text-text-light py-20 px-6 text-center overflow-hidden">
      <div class="absolute inset-0 z-0 opacity-30 animate-background-pan" style="background-image: url('https://images.unsplash.com/photo-1519389950473-47ba0277781c?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1470&q=80'); background-size: cover; background-position: center;"></div>
      <div class="relative z-10 container mx-auto animate-fade-in-up" style="animation-delay: 0.1s;">
        ...
        />
        <h1 class="text-5xl md:text-6xl font-extrabold mb-4 leading-tight">{{ company?.name || 'Fola Company' }}</h1>
        <p class="text-xl md:text-2xl mb-8 max-w-3xl mx-auto">{{ company?.description || 'Discover our amazing products and services.' }}</p>
        <RouterLink
          to="/products"
    ...
    <div class="container mx-auto p-6">
      ...
      <section class="my-16 animate-fade-in-up" style="animation-delay: 0.3s;">
        ...
        <div class="bg-secondary-black p-8 rounded-lg shadow-xl border border-border-color max-w-4xl mx-auto">
          <p class="text-text-dark leading-relaxed text-lg text-center">
            Fola Company is dedicated to providing high-quality products that meet your everyday needs.
            We believe in innovation, customer satisfaction, and sustainable practices. Our mission is
            to enrich lives through exceptional products and unparalleled service. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed do eiusmod tempor incididunt ut
      ...
      <section class="my-16 animate-fade-in-up" style="animation-delay: 0.5s;">
        <h2 class="text-text-light text-4xl font-bold mb-8 text-center">Featured Products</h2>
        ...
        >
          <ProductCard v-for="(product, index) in latestProducts" :key="product.id" :product="product" class="animate-fade-in-up" :style="{ 'animation-delay': `${0.6 + index * 0.1}s` }" />
        </div>
        <p v-else class="text-text-dark text-center py-8 bg-secondary-black p-6 rounded-lg shadow-md border border-border-color">No featured products available.</p>
      </section>
    </div>
  </div>
</template>
